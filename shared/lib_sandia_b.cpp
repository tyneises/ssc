...
sandia_module_t_b::sandia_module_t_b()
{
	A0 = A1 = A2 = A3 = A4
		= B0 = B1 = B2 = B3 = B4 = B5
...
bool sandia_module_t_b::operator() (pvinput_t &in, double TcellC, double opvoltage, pvoutput_t &out)
{
...
sandia_inverter_t_b::sandia_inverter_t_b()
{
	Paco = Pdco = Vdco = Pso
		= Pntare = C0 = C1 = C2 = C3 = std::numeric_limits<double>::quiet_NaN();
}

bool sandia_inverter_t_b::acpower(
	util::matrix_t<ssc_number_t> &Pdc,     /* Input power to inverter (Wdc) assumed to be intire inverter bank input */
	util::matrix_t<ssc_number_t> &Vdc,     /* Voltage input to inverter (Vdc) */
	int &num_inverter, /* number of inverters in the inverter bank, will need to decide how to handle for different multiple inverters */

	/* outputs */
	util::matrix_t<ssc_number_t> &Pac,    /* AC output power (Wac) */
	util::matrix_t<ssc_number_t> &Ppar,   /* AC parasitic power consumption (Wac) */
	util::matrix_t<ssc_number_t> &Plr,    /* Part load ratio (Pdc_in/Pdc_rated, 0..1) */
	util::matrix_t<ssc_number_t> &Eff,	    /* Conversion efficiency (0..1) */
	util::matrix_t<ssc_number_t> &Pcliploss, /* Power loss due to clipping loss (Wac) */
	util::matrix_t<ssc_number_t> &Psoloss, /* Power loss due to operating power consumption (Wdc) */
	util::matrix_t<ssc_number_t> &Pntloss /* Power loss due to night time tare loss (Wac) */
	)
{
	for (size_t i = 0; i < Pdc.length(); i++)
	{
		double A = Pdco * (1.0 + C1*(Vdc.at(i) - Vdco));
		double B = Pso * (1.0 + C2*(Vdc.at(i) - Vdco));
		double C = C0 * (1.0 + C3*(Vdc.at(i) - Vdco));
		double Pin = Pdc.at(i);
		// equally divide power by all inverter (voltage remains the same)
		if (num_inverter > 0) Pin /= num_inverter;
		...
		Pac.at(i) = ((Paco / (A - B)) - C*(A - B))*(Pin - B) + C0*(Pin - B)*(Pin - B);
		Ppar.at(i) = 0.0;
		...
		else
		{
			...
			double PacNoPso = ((Paco / A) - C*A)*Pin + C0*Pin*Pin;
			Psoloss.at(i) = PacNoPso - Pac.at(i);
		}

		// clipping loss Wac (note that the Pso=0 may have no clipping)
		Pcliploss.at(i) = 0.0;
		double PacNoClip = Pac.at(i);
		if (Pac.at(i) > Paco)
		{
			Pac.at(i) = Paco;
			Pcliploss.at(i) = PacNoClip - Pac.at(i);
		}

		Plr.at(i) = Pin / Pdco;
		Eff.at(i) = Pac.at(i) / Pin;
		if (Eff.at(i) < 0.0) Eff.at(i) = 0.0;

		// update all outputs by number of inverter
		if (num_inverter > 0)
		{
			Pac.at(i) *= num_inverter;
			Ppar.at(i) *= num_inverter;
			Plr.at(i) *= num_inverter;
			Eff.at(i) *= 100.0;
			Pcliploss.at(i) *= num_inverter;
			Psoloss.at(i) *= num_inverter;
			Pntloss.at(i) *= num_inverter;
		}
	}
	return true;
}
...
double sandia_celltemp_t_b::sandia_tcell_from_tmodule(double Tm, double poaIrr, double fd, double DT0)
{
	/*
...
double sandia_celltemp_t_b::sandia_module_temperature(double poaIrr, double Ws, double Ta, double fd, double a, double b)
{
	/*
...
bool sandia_celltemp_t_b::operator() (pvinput_t &input, pvmodule_t &module, double opvoltage, double &Tcell)
{
